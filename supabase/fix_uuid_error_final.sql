-- FINAL FIX: UUID Type Casting
-- This script fixes the "operator does not exist: uuid = text" error by explicitly casting all IDs
-- It also cleans up the debug logging table

-- 1. Cleanup debug table
...
-- 3. Recreate the function with EXPLICIT UUID CASTS
...
BEGIN
  ...
  BEGIN
    ...
    -- Get business owner and name (FIX: Explicit cast to uuid)
    SELECT owner_id, name INTO v_business_owner_id, v_business_name
    FROM public.businesses
    WHERE id = NEW.business_id::uuid;
    
    -- Get driver user_id if assigned (FIX: Explicit cast to uuid)
    IF NEW.driver_id IS NOT NULL THEN
      SELECT user_id INTO v_driver_user_id
      FROM public.drivers
      WHERE id = NEW.driver_id::uuid;
    END IF;
